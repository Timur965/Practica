...
void Operation::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)               //Отрисовываем операцию
{
    ...
    start = sceneWidth/2 + this->pos().x()-coordStart;
    end = (sceneWidth/2 + this->pos().x()+this->width)-coordStart;
    if(move)
    {
        painter->drawText(0,rect.y()-height/2-10,rect.width()+75,rect.height(),Qt::AlignLeft,"Начало: "+QString::number(start));
        painter->drawText(0,rect.y()+height/2+20,rect.width()+75,rect.height(),Qt::AlignLeft,"Конец: "+QString::number(end));
    }
    if(inQueue)
        brush=Qt::magenta;
