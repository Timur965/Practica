...
void MainWindow::on_pushButton_clicked()                                                   //Кнопка добавления
{
    ...
    {                                                                                      //Проверка что вводятся цифры
        if(ui->coordX->text().toDouble() > (ui->graphicsView->width()/2 - ui->graphicsView->width())+55 &&
           ...
           ui->operationWidth->text().toDouble() >=0 &&
           ui->operationWidth->text().toDouble() < ui->graphicsView->width() &&
           ui->operationHeight->text().toDouble() >=0 &&
        ...
        {
            nameOperations = scene->addOperations(ui->coordX->text().toDouble(),ui->coordY->text().toDouble(),ui->operationWidth->text().toDouble(),ui->operationHeight->text().toDouble());
            ui->comboBox->addItem(nameOperations);
        }
        else
        {
...
void MainWindow::on_pushButton_2_clicked()                                                 //Кнопка изменения
{
    ...
    {                                                                                      //Проверка что вводятся цифры
        if(ui->newCoordX->text().toDouble() > (ui->graphicsView->width()/2 - ui->graphicsView->width())+55 &&
           ...
           ui->newOperationWidth->text().toDouble() >=0 &&
           ui->newOperationWidth->text().toDouble() < ui->graphicsView->width() &&
           ui->newOperationHeight->text().toDouble() >=0 &&
           ui->newOperationHeight->text().toDouble() < ui->graphicsView->height())          //Проверка чтобы вводимые данные не выходили за границы представления
        {
            ...
            {
                scene->updateOperations(ui->comboBox->currentIndex(),ui->newCoordX->text().toDouble(),ui->newCoordY->text().toDouble(),ui->newOperationWidth->text().toDouble(),ui->newOperationHeight->text().toDouble());
            }
        }
        else
        {
...
bool MainWindow::eventFilter(QObject *object, QEvent *event)
{
    ...
    if(event->type() == QEvent::MouseButtonDblClick)
    {
        if(object == ui->graphicsView)
        {
            ...
            {
                static int id = 1;
                nameOperations = scene->addOperations();
                if(nameOperations.contains("Operation"))
                    ui->comboBox->addItem(nameOperations);
                else
                    QMessageBox::warning(this,"Добавление",nameOperations);
                id++;
            }
        }
    }
